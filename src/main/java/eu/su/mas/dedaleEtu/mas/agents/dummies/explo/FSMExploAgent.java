...
public class FSMExploAgent extends AbstractDedaleAgent {
    ...
    private final Map<String, Integer> failedStenchMoves = new ConcurrentHashMap<>();
    private static final int GOLEM_INFERENCE_FAILED_STENCH_THRESHOLD = 2;
    // ===========================================
    ...
    public synchronized void addOrUpdateGolem(String golemId, String position, boolean confirmed) {
        if (!confirmed && isInferredGolemId(golemId) && isOccupiedByAgent(position)) {
            knownGolems.remove(golemId);
            if (golemId.equals(currentTargetGolemId)) {
                currentTargetGolemId = null;
            }
            System.out.println(getLocalName() + " ignored inferred Golem at occupied position: " + golemId);
            return;
        }
    ...
    public synchronized boolean registerFailedStenchMove(String nodeId) {
        int failures = failedStenchMoves.merge(nodeId, 1, Integer::sum);
        System.out.println(getLocalName() + " stench move failure on " + nodeId + " count=" + failures);
        return failures >= GOLEM_INFERENCE_FAILED_STENCH_THRESHOLD;
    }

    public synchronized void clearFailedStenchMove(String nodeId) {
        if (nodeId != null) {
            failedStenchMoves.remove(nodeId);
        }
    }

    public String getInferredGolemId(String nodeId) {
        return "InferredGolem@" + nodeId;
    }

    public synchronized void pruneInvalidInferredGolemsAt(String position) {
        if (position == null) return;
        List<String> toRemove = new ArrayList<>();
        for (GolemInfo golem : knownGolems.values()) {
            if (!golem.isConfirmed()
                    && isInferredGolemId(golem.getId())
                    && position.equals(golem.getLastKnownPosition())) {
                toRemove.add(golem.getId());
            }
        }

        for (String golemId : toRemove) {
            knownGolems.remove(golemId);
            capturedGolems.remove(golemId);
            if (golemId.equals(currentTargetGolemId)) {
                currentTargetGolemId = null;
            }
            blockingTargets.remove(golemId);
            System.out.println(getLocalName() + " removed invalid inferred Golem at own position: " + golemId);
        }
    }

    public synchronized void pruneInvalidInferredGolemsAtKnownAgentPositions() {
        Set<String> occupied = getKnownAgentPositions();
        Location current = getCurrentPosition();
        if (current != null) {
            occupied.add(current.getLocationId());
        }
        ...
        for (GolemInfo golem : knownGolems.values()) {
            if (!golem.isConfirmed()
                    && isInferredGolemId(golem.getId())
                    && occupied.contains(golem.getLastKnownPosition())) {
                toRemove.add(golem.getId());
            }
        }
        ...
        for (String golemId : toRemove) {
            ...
            blockingTargets.remove(golemId);
            System.out.println(getLocalName() + " removed invalid inferred Golem at occupied position: " + golemId);
        }
    }

    public boolean isInferredGolemId(String golemId) {
        return golemId != null && golemId.startsWith("InferredGolem@");
    }

    public boolean isGolemAgentName(String agentName) {
        if (agentName == null) {
            return false;
        }
        String lowerName = agentName.toLowerCase();
        return lowerName.contains("wumpus")
                || lowerName.contains("golem")
                || agentName.equalsIgnoreCase("ImHere");
    }

    private boolean isCurrentPosition(String position) {
        Location current = getCurrentPosition();
        return current != null && position != null && position.equals(current.getLocationId());
    }

    private boolean isOccupiedByAgent(String position) {
        return isCurrentPosition(position) || getKnownAgentPositions().contains(position);
    }

    public synchronized void markGolemCaptured(String golemId) {
        if (isInferredGolemId(golemId)) {
            capturedGolems.remove(golemId);
            knownGolems.remove(golemId);
            if (golemId.equals(currentTargetGolemId)) {
                currentTargetGolemId = null;
            ...
            blockingTargets.remove(golemId);
            System.out.println(getLocalName() + " ignored capture for unconfirmed inferred Golem: " + golemId);
        ...
        if (knownGolems.isEmpty() && myMap != null) {
    ...
    public GolemInfo selectBestTarget(String myPos) {
        ...
        return knownGolems.values().stream()
                ...
                .filter(GolemInfo::isConfirmed)
                .filter(g -> myPos.equals(g.getLastKnownPosition())
                        || myMap.getShortestPath(myPos, g.getLastKnownPosition()) != null)
