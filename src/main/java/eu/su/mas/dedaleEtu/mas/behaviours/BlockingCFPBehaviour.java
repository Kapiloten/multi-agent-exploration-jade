...
public class BlockingCFPBehaviour extends OneShotBehaviour {
    ...
    public void action() {
        ...
        String myAssignment = findAssignedNode(assignments, agent.getLocalName());
    ...
    private String findAssignedNode(Map<String, String> assignments, String agentName) {
        for (Map.Entry<String, String> assignment : assignments.entrySet()) {
            if (agentName.equals(assignment.getValue())) {
    ...
    private void processProposal(ACLMessage msg) {
        ...
        String[] nodeCostPairs = parts[1].split(",");
    ...
    private Map<String, String> decideAssignments() {
        ...
        Set<String> occupiedByOthers = agent.getKnownAgentPositions();
        ...
        for (String node : neighbors) {
            if (node.equals(myPos) && !assignments.containsValue(agent.getLocalName())) {
            ...
            if (!occupiedByOthers.contains(node) && !assignments.containsValue(agent.getLocalName())) {
                assignments.put(node, agent.getLocalName());
            }
        }
        return assignments;
